...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "artifacts" / "node_stores_inference_qwen"
...
def get_store_dir(
    ...
    return Path(output_root) / f"{image_path.stem}_qwen"
...
def ensure_offline_store_qwen(
    ...
    qwen_model: str = DEFAULT_QWEN_VL_MODEL,
    ...
    resolved_qwen_crops_dir = resolve_optional_output_dir(qwen_crops_dir)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build or refresh one Qwen offline store for a document image."
    )
    ...
    parser.add_argument("--qwen-model", default=DEFAULT_QWEN_VL_MODEL, help="Qwen model for region analysis.")
    ...
    parser.add_argument("--qwen-max-new-tokens", type=int, default=1024, help="Max tokens for Qwen region analysis.")
    parser.add_argument("--qwen-temperature", type=float, default=0.1, help="Qwen temperature for region analysis.")
